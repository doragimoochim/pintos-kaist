...
file_backed_initializer (struct page *page, enum vm_type type, void *kva) {
	/* Set up the handler */
	// 먼저 page->operations에 file-backed pages에 대한 핸들러 설정
...
file_backed_destroy (struct page *page) {
	// page struct를 해제할 필요는 없다(file_backed_destroy의 호출자가 해야 함)
	struct file_page *file_page UNUSED = &page->file;
	if (pml4_is_dirty(thread_current()->pml4, page->va))
	{
		file_write_at(file_page->file, page->va, file_page->read_bytes, file_page->ofs);
...
do_mmap (void *addr, size_t length, int writable,
		struct file *file, off_t offset){
			...
			ASSERT(pg_ofs(addr) == 0);	//upage가 페이지정렬되어 있는지 확인
			ASSERT(offset % PGSIZE == 0);	//ofs가 페이지정렬되어 있는지 확인

			while (read_bytes > 0 || zero_bytes > 0)
			{
				/* 이 페이지를 채우는 방법을 계산
				파일에서 PAGE_ZERO_BYTES 바이트를 읽고
				...
				// vm_alloc_page_with_initializer를 호출하여 대기 중인 객체 생성
				...
				/* Advance */
				// 읽은 바이트와 0으로 채운 바이트를 추적하고 가상 주소를 증가시킴
