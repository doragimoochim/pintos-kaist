...
vm_anon_init (void) {
	...
	// : 1 sector = 512bytes, 1page = 4096bytes -> 1 slot = 8 sector
...
// 이 함수는 익명 페이지의 핸들러를 page->operations에 설정
// 현재는 빈 구조체인 anon_page에서 일부 정보를 업데이트 해야 할 수도 있습니다.
// 이 함수는 이겸ㅇ 페이지(즉, VM_ANON)의 초기화 함수로 사용됨
bool
...
anon_swap_in (struct page *page, void *kva) {
	...
	for (e = list_begin(&swap_table); e != list_end(&swap_table); e = list_next(e))
	{
		...
		if (slot->slot_no == page_slot_no)	// 현재 page가 사용중인 slot 찾기
		{
			for (int i = 0; i < 8; i++)
			{
				// 디스크, 읽을섹터 번호, 담을주소 (512bytes씩 읽는다. disk관련은 동기화 처리가 되어 있어서 lock불필요)
...
/* 익명 페이지를 destroy함. page는 호출자에 의해 해제*/
static void
anon_destroy (struct page *page) {
	struct anon_page *anon_page = &page->anon;
	// anonymous page에 의해 유지되던 리소스를 해제
	// page struct를 명시적으로 해제할 필요는 없음, 호출자가 이를 수행
	struct list_elem *e;
	struct slot *slot;
